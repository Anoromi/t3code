...
import {
  ...
  extractWorkspaceIdFromHyprnavStdout,
...
function createCliDeps(input: {
  ...
  readonly launchWorkspace?: number;
  ...
  return {
    deps: {
      ...
      launchGhostty: (command: string) => {
        dispatches.push(`launch:${command}`);
        if (input.launchError) {
          throw input.launchError;
        }
        return input.launchWorkspace ?? 17;
      },
...
describe("ghostty-worktree", () => {
  ...
  it("parses the first non-empty hyprnav workspace id line", () => {
    expect(extractWorkspaceIdFromHyprnavStdout("\n17\n")).toBe(17);
    expect(extractWorkspaceIdFromHyprnavStdout("workspace\n17\n")).toBeNull();
    expect(extractWorkspaceIdFromHyprnavStdout("")).toBeNull();
  });
  ...
  it("builds a Ghostty launch command", () => {
  ...
  it("parses list-open CLI invocation", () => {
    expect(parseCliArgs(["list-open"])).toEqual({
      mode: "list-open",
      execCommand: null,
    });
  });
  ...
  it("rejects invalid CLI arguments", () => {
    expect(() => parseCliArgs(["--exec"])).toThrow(
      "ghostty-worktree only accepts no args, 'list-open', or '--exec <command>'.",
    );
  ...
  it("creates a new Ghostty on a random workspace when the saved pid is stale", async () => {
    const worktree = createWorktree();
    const assignment = createAssignment(worktree, { pid: 200 });
    ...
    expect(result.exitCode).toBe(0);
    expect(dispatches[0]).toContain("launch:sh -lc");
    expect(dispatches.slice(1)).toEqual(["workspace:17", "focus:0x1234"]);
    expect(outputs.stdout).toEqual([`pid=4321 workspace=17 worktree=${worktree.worktreeRoot}`]);
  ...
  it("launches Ghostty with the managed class, title, and caller cwd", async () => {
    const worktree = createWorktree({
  ...
  it("trusts the live client workspace when it differs from the hyprnav launch workspace", async () => {
    const worktree = createWorktree();
    const { deps, dispatches } = createCliDeps({
      argvWorktree: worktree,
      clients: [
    ...
    expect(dispatches).toEqual([
      expect.stringContaining("launch:sh -lc"),
      "workspace:4",
      "focus:0x1234",
    ]);
  });

  it("lists open managed Ghostty worktrees", async () => {
    const worktree = createWorktree();
    const { deps, outputs, fs, stateFilePath } = createCliDeps({
      registry: {
        version: 1,
        assignments: {
          [worktree.key]: createAssignment(worktree, { pid: 200 }),
        },
      },
      clients: [
        [createClient({ pid: 200, className: createManagedClassName(worktree.key), workspace: 9 })],
      ],
    });

    const result = await runCli(["list-open"], deps);

    expect(result.exitCode).toBe(0);
    expect(outputs.stdout).toEqual([JSON.stringify([{ worktreePath: worktree.worktreeRoot }])]);
    expect(fs.files.get(stateFilePath)).toContain('"pid": 200');
  });

  it("omits stale managed Ghostty assignments from list-open", async () => {
    const worktree = createWorktree();
    ...
    expect(result.exitCode).toBe(0);
    expect(outputs.stdout).toEqual([JSON.stringify([])]);
  });

  it("fails clearly when hyprnav does not report a workspace id", async () => {
    const { deps, outputs } = createCliDeps({
      launchError: new Error("hyprnav did not report a workspace id."),
    });

    const result = await runCli([], deps);

    expect(result.exitCode).toBe(1);
    expect(outputs.stderr[0]).toContain("hyprnav did not report a workspace id");
  });
