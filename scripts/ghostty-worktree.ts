...
interface CliDeps {
  ...
  readonly launchGhostty: (command: string) => number;
...
export function parseCliArgs(argv: ReadonlyArray<string>): ParsedCliArgs {
  if (argv.length === 0) {
    return { mode: "open", execCommand: null };
  }
  ...
  if (argv.length === 2 && argv[0] === "--exec") {
    ...
    return { mode: "open", execCommand };
  }

  if (argv.length === 1 && argv[0] === "list-open") {
    return { mode: "list-open", execCommand: null };
  }
...
export function extractWorkspaceIdFromHyprnavStdout(stdout: string): number | null {
...
export function extractWorkspaceIdForHyprnavSpawnSocketFallback(input: {
  ...
  return extractWorkspaceIdFromHyprnavStdout(input.stdout);
}
...
function resolveLiveAssignment(
  ...
  readonly assignment: GhosttyAssignment;
  readonly client: HyprClient;
} | null {
  const liveClient = findManagedClient(clients, assignment);
  if (liveClient !== null) {
    return {
      assignment,
      client: liveClient,
  ...
  const recoveredClient = findManagedClientByClassName(clients, assignment.className);
    ...
    launchGhostty: (command) => {
      ...
      if (result.error) {
        if ((result.error as NodeJS.ErrnoException).code === "ENOENT") {
          throw new Error("hyprnav does not appear to be installed in PATH.");
        }

        throw new Error(`Unable to execute hyprnav: ${String(result.error)}.`);
      }

      if (result.status !== 0) {
        const stderr = result.stderr.trim();
        const fallbackWorkspace = extractWorkspaceIdForHyprnavSpawnSocketFallback({
          stdout: result.stdout,
          stderr,
        });
        if (fallbackWorkspace !== null) {
          runTextCommand("hyprctl", ["dispatch", "workspace", String(fallbackWorkspace)]);
          runTextCommand("hyprctl", ["dispatch", "exec", command]);
          return fallbackWorkspace;
        }

        throw new Error(
          stderr.length > 0 ? stderr : `hyprnav spawn failed with status ${String(result.status)}.`,
        );
      }

      const workspaceId = extractWorkspaceIdFromHyprnavStdout(result.stdout);
      if (workspaceId === null) {
        throw new Error("hyprnav did not report a workspace id.");
      }
...
export async function runCli(argv: ReadonlyArray<string>, deps: CliDeps): Promise<CliResult> {
  ...
  try {
    ...
    if (parsedArgs.mode === "list-open") {
      const clients = deps.listClients();
      const nextAssignments: Record<string, GhosttyAssignment> = {};
      const openWorktrees: OpenWorktreeTerminalEntry[] = [];

      for (const assignment of Object.values(prunedRegistry.assignments)) {
        const liveAssignment = resolveLiveAssignment(assignment, clients);
        if (liveAssignment === null) {
          continue;
        }

        const key = createAssignmentKey(
          liveAssignment.assignment.repoCommonDir,
          liveAssignment.assignment.worktreeRoot,
        );
        nextAssignments[key] = liveAssignment.assignment;
        openWorktrees.push({
          worktreePath: liveAssignment.assignment.worktreeRoot,
        });
      }

      const nextRegistry: GhosttyRegistry = {
        version: 1,
        assignments: nextAssignments,
      };
      if (serializeRegistry(nextRegistry) !== serializeRegistry(loadedRegistry)) {
        saveRegistry(stateFilePath, nextRegistry, deps.fileSystem);
      }

      writeStdout(`${JSON.stringify(openWorktrees)}\n`.trimEnd());
    ...
    if (existingAssignment) {
      const liveAssignment = resolveLiveAssignment(existingAssignment, deps.listClients());
      if (liveAssignment !== null) {
        nextRegistry =
          liveAssignment.assignment === existingAssignment
            ? prunedRegistry
            : {
                version: 1,
                assignments: {
                  ...prunedRegistry.assignments,
                  [worktree.key]: liveAssignment.assignment,
                },
              };
        ...
        deps.dispatchWorkspace(liveAssignment.client.workspace);
        deps.dispatchFocusWindow(liveAssignment.client.address);
        writeStdout(
          formatAssignmentLine(
            liveAssignment.assignment.pid,
            liveAssignment.client.workspace,
    ...
    try {
      ...
      deps.dispatchWorkspace(client.workspace);
      deps.dispatchFocusWindow(client.address);
      writeStdout(formatAssignmentLine(client.pid, client.workspace, worktree.worktreeRoot));
      return { exitCode: 0, stdout, stderr };
    } catch (error) {
      if (stateChanged || nextRegistry !== prunedRegistry) {
        saveRegistry(stateFilePath, nextRegistry, deps.fileSystem);
