...
2. Recover branch commit intent before reimplementing.
   For each paused rebase commit or conflict cluster, first try to find a matching commit note at `commits/<full commit subject>.md`.
   Use the commit subject, not the hash, because hashes may change during rebases and amendments.
