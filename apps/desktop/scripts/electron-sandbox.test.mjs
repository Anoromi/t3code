...
function evaluateSandbox({
  ...
  NodeFS.writeFileSync(unsharePath, `#!/bin/sh\nexit ${unshareWorks ? 0 : 1}\n`, {
    mode: 0o755,
  });
  if (helperExists) {
...
describe("Nix Electron sandbox selection", () => {
  it("prefers an executable system sandbox helper", () => {
  ...
  it("uses the user-namespace sandbox when available", () => {
  ...
  it("rejects a system helper without root ownership and mode 4755", () => {
    assert.deepEqual(
      ...
      ["args=--disable-setuid-sandbox", "helper=unset"],
    );
  });

  it("starts without a sandbox when the host supports neither strategy", () => {
