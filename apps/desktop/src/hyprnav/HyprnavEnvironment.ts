...
export function buildWorktreeTerminalCommand(environmentPath: string): string {
  return [
    "exec ghostty",
    "--gtk-single-instance=false",
    `--working-directory=${quoteShellArg(environmentPath)}`,
    "-e sh -lc",
    quoteShellArg("exec tmux"),
  ].join(" ");
}
...
function buildCorkdiffCommand(input: {
  ...
  const env = [
    ...
    ...(input.connection.token ? [`T3CODE_TOKEN=${quoteShellArg(input.connection.token)}`] : []),
  ];
  return [
    "cd",
    ...
    ...env,
    "exec ghostty",
    ...buildCorkdiffGhosttyArgs({ className, threadId: input.threadId }).map(quoteShellArg),
  ].join(" ");
}
...
export class HyprnavEnvironmentManager {
  ...
  lock(input: DesktopHyprnavLockInput): Promise<DesktopHyprnavSyncResult> {
    ...
    return this.serialize(`lock:${envId}`, () => this.run(["lock", envId]));
  }
  ...
  private resolveBinding(
    ...
    switch (binding.action) {
      case "worktree-terminal":
        command = buildWorktreeTerminalCommand(environmentPath);
        break;
      case "open-favorite-editor":
        command = buildEditorCommand(environmentPath, input.preferredEditor);
        if (command === null) {
          return {
            status: "unavailable",
  ...
  private async performSync(input: CanonicalSyncInput): Promise<DesktopHyprnavSyncResult> {
    ...
    if (syncResult.status !== "ok" || !input.lock) return bindingError ?? syncResult;
    const lockResult = await this.run(["lock", ids.lockEnvId]);
    return bindingError ?? lockResult;
  }

  private async run(args: readonly string[], stdin?: string): Promise<DesktopHyprnavSyncResult> {
    try {
      const result = await new Promise<{
        ...
        const child = this.spawn("hyprnav", [...args], {
      ...
      const rendered = ["hyprnav", ...args].map(formatArg).join(" ");
