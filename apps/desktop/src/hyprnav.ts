...
function isHyprnavUnavailableError(error: unknown): boolean {
  return (
...
export class HyprnavEnvironmentSync {
  ...
  private async runPending(syncKey: string, initialPending: PendingHyprnavSync): Promise<void> {
    ...
    try {
      const result = await this.performSync(pending.input);
  ...
  private async runPendingLock(envId: string, initialPending: PendingHyprnavLock): Promise<void> {
    ...
    try {
      const result = await this.runHyprnav(["lock", pending.envId]);
      for (const waiter of pending.waiters) {
        waiter.resolve(result);
      }
    } catch (error) {
  ...
  private async performSync(input: CanonicalHyprnavSyncInput): Promise<DesktopHyprnavSyncResult> {
    const envIds = buildHyprnavEnvironmentIds(input);
    const bindingsResult = this.collectHyprnavBindings(input, envIds);
    if (bindingsResult.tag === "error" && !input.lock) {
    ...
    const syncResult =
      operations.length === 0
        ? ({ status: "ok", message: null } satisfies DesktopHyprnavSyncResult)
        : await this.runHyprnavBatch({
            atomic: true,
            operations,
          });

    if (input.lock) {
      const lockResult = await this.runHyprnav(["lock", envIds.lockEnvId]);
    ...
    return bindingsResult.tag === "error" ? bindingsResult.result : syncResult;
  }
  ...
  private async runHyprnav(args: readonly string[]): Promise<DesktopHyprnavSyncResult> {
    return await this.runHyprnavCommand({
      args,
      stdio: ["ignore", "pipe", "pipe"],
    });
  }

  private async runHyprnavBatch(payload: HyprnavBatchPayload): Promise<DesktopHyprnavSyncResult> {
  ...
  private async runHyprnavCommand(input: {
    readonly args: readonly string[];
    readonly stdio: ["ignore" | "pipe", "pipe", "pipe"];
    readonly input?: string;
  }): Promise<DesktopHyprnavSyncResult> {
    const startedAt = Date.now();
    const renderedCommand = ["hyprnav", ...input.args].map(formatArgForDisplay).join(" ");
    this.deps.log?.(`hyprnav start command=${renderedCommand}`);
    ...
    try {
      const result = await new Promise<{
        ...
        const child = this.deps.spawn("hyprnav", [...input.args], {
          stdio: input.stdio,
        });
      ...
      if (result.timedOut) {
        this.deps.log?.(
          `hyprnav timeout durationMs=${String(durationMs)} command=${renderedCommand}`,
        );
        return {
          status: "error",
          message: `${renderedCommand} timed out.`,
        };
      }

      if (result.code === 0) {
        this.deps.log?.(`hyprnav ok durationMs=${String(durationMs)} command=${renderedCommand}`);
        return { status: "ok", message: null };
      }

      this.deps.log?.(`hyprnav error durationMs=${String(durationMs)} command=${renderedCommand}`);
      return {
        status: "error",
        message: formatCommandFailure("hyprnav", input.args, result.stderr.trim()),
      };
    } catch (error) {
      const durationMs = Date.now() - startedAt;
      if (isHyprnavUnavailableError(error)) {
        this.deps.log?.(
          `hyprnav unavailable durationMs=${String(durationMs)} command=${renderedCommand}`,
        );
        return {
          status: "unavailable",
      ...
      this.deps.log?.(
        `hyprnav spawn-error durationMs=${String(durationMs)} command=${renderedCommand} message=${message}`,
      );
      return {
        status: "error",
        message: `${renderedCommand}: ${message}`,
      };
    }
  }
}
