...
function makeChatMessage(overrides: {
  ...
  return {
    id: MessageId.make(overrides.id ?? crypto.randomUUID()),
...
describe("deriveRecoverableUserInputPrompt", () => {
  ...
  it("accepts both unknown-pending detail variants", () => {
    ...
    const codex = deriveRecoverableUserInputPrompt(
      [
        makeActivity({
          id: "open-codex",
          ...
          tone: "info",
          payload: {
            requestId: "req-user-input-codex",
            questions: [
              {
                id: "sandbox_mode",
                header: "Sandbox",
        ...
        makeActivity({
          ...
          payload: {
            requestId: "req-user-input-codex",
            detail: "Unknown pending Codex user input request: req-user-input-codex",
          },
        }),
      ],
      [],
    ...
    expect(codex?.requestId).toBe("req-user-input-codex");
  });
...
describe("deriveVisiblePendingUserInputs", () => {
  it("surfaces recoverable Codex stale prompts to the composer", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "live-open-codex",
        createdAt: "2026-06-05T18:04:00.000Z",
        kind: "user-input.requested",
        summary: "User input requested",
        tone: "info",
        payload: {
          requestId: "fb4d2e83-2d23-4d62-bbbc-f51f5ba21e13",
          questions: [
            {
              id: "choice",
              header: "Action",
      ...
      makeActivity({
        ...
        payload: {
          requestId: "fb4d2e83-2d23-4d62-bbbc-f51f5ba21e13",
          detail: "Unknown pending Codex user input request: fb4d2e83-2d23-4d62-bbbc-f51f5ba21e13",
        },
      }),
    ];

    expect(derivePendingUserInputs(activities)).toEqual([]);
    expect(deriveVisiblePendingUserInputs(activities, [], makeThreadSession())).toEqual([
