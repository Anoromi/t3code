...
export interface WsRpcClient {
  ...
  readonly readAloud: {
    readonly synthesize: RpcUnaryMethod<typeof WS_METHODS.readAloudSynthesize>;
  };
...
export function createWsRpcClient(transport: WsTransport): WsRpcClient {
  return {
    ...
    readAloud: {
      synthesize: (input) =>
        transport.request((client) => client[WS_METHODS.readAloudSynthesize](input)),
    },
    sourceControl: {
    ...
    desktop: {
      ...
      onControlEvent: (listener, options) =>
        transport.subscribe(
          (client) => client[WS_METHODS.subscribeDesktopControl]({}),
          listener,
          options,
        ),
    },
