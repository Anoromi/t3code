...
export default function EmbeddedCorkdiffPanel({
  ...
  const threadRef = environmentId ? scopeThreadRef(environmentId, threadId) : null;
  const thread = useStore((store) => selectThreadByRef(store, threadRef));
  const project = useStore((store) =>
    thread
      ? selectProjectByRef(store, scopeProjectRef(thread.environmentId, thread.projectId))
      : undefined,
  );
  ...
  const rawWsUrl = window.desktopBridge?.getLocalEnvironmentBootstrap()?.wsBaseUrl ?? null;
  ...
  const canRenderTerminal = launched && threadRef !== null;
