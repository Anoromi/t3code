...
export interface TerminalViewportProps {
  ...
  command?: TerminalOpenInput["command"];
  onSessionExited: () => void;
  onAddTerminalContext: (selection: TerminalContextSelection) => void;
  onSessionSnapshot?: (snapshot: TerminalSessionSnapshot) => void;
  onSessionEvent?: (event: TerminalEvent) => void;
  onOpenError?: (message: string) => void;
  eventSubscription?: (listener: (event: TerminalEvent) => void) => () => void;
  eventEntriesSelector?: () => ReadonlyArray<TerminalEventEntry>;
  eventStoreSubscribe?: (listener: TerminalEventEntriesListener) => () => void;
...
export type TerminalEventEntriesListener = (
  nextEntries: ReadonlyArray<TerminalEventEntry>,
  previousEntries: ReadonlyArray<TerminalEventEntry>,
) => void;
...
export function maxDrawerHeight(): number {
  if (typeof window === "undefined") return MIN_DRAWER_HEIGHT;
  return Math.max(MIN_DRAWER_HEIGHT, Math.floor(window.innerHeight * MAX_DRAWER_HEIGHT_RATIO));
}

export function clampDrawerHeight(height: number, fallbackHeight: number): number {
  const safeHeight = Number.isFinite(height) ? height : fallbackHeight;
...
export function terminalThemeFromApp(surfaceElement?: HTMLElement | null): TerminalTheme {
  const isDark = document.documentElement.classList.contains("dark");
  const themeSurface =
    surfaceElement?.closest<HTMLElement>(".thread-terminal-drawer") ?? document.body;
  const bodyStyles = getComputedStyle(themeSurface);
  const background =
...
export function describeTerminalExit(event: Extract<TerminalEvent, { type: "exited" }>): string {
  const details = [
    ...
    .join(", ");
  return details.length > 0 ? `Process exited (${details})` : "Process exited";
}
