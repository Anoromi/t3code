...
import {
  invalidateServerAuthBootstrapCache,
...
async function issueDesktopWebSocketToken(_httpBaseUrl: string): Promise<string> {
  let response = await fetch(resolvePrimaryEnvironmentHttpUrl("/api/auth/ws-token"), {
    credentials: "include",
    method: "POST",
  });

  if (response.status === 401 && window.desktopBridge?.getLocalEnvironmentBootstrap()) {
    invalidateServerAuthBootstrapCache();
    await resolveInitialServerAuthGateState();
    response = await fetch(resolvePrimaryEnvironmentHttpUrl("/api/auth/ws-token"), {
      credentials: "include",
      method: "POST",
    });
  }
