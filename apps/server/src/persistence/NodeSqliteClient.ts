  ...
  const makeConnection = Effect.gen(function* () {
    ...
    const prepareCache = yield* Cache.make({
      ...
      lookup: (sql: string) =>
        Effect.try({
          try: () => db.prepare(sql),
          catch: (cause) => toSqlError("Failed to prepare statement", cause),
        }),
    });
    ...
    const runStatement = (statement: StatementSync, params: ReadonlyArray<unknown>, raw: boolean) =>
      Effect.withFiber<ReadonlyArray<any>, SqlError>((fiber) => {
        ...
        try {
          ...
          return Effect.fail(toSqlError("Failed to execute statement", cause));
        }
      });
      ...
      Effect.acquireUseRelease(
        ...
        (statement) =>
          Effect.try({
            ...
            catch: (cause) => toSqlError("Failed to execute statement", cause),
          }),
        (statement) =>
          Effect.sync(() => {
            if (hasRows(statement)) {
