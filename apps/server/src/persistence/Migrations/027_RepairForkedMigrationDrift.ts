...
const getProjectionProjectColumns = (sql: SqlClient.SqlClient) =>
  sql`PRAGMA table_info(projection_projects)`.values.pipe(
    Effect.map(
...
export default Effect.gen(function* () {
  ...
  const projectColumns = yield* getProjectionProjectColumns(sql);
  if (!projectColumns.has("default_model_selection_json")) {
    yield* sql`
  ...
  const threadColumns = yield* getProjectionThreadColumns(sql);
  ...
  const forkColumns = yield* getProjectionThreadColumns(sql);

  if (forkColumns.has("source_thread_id") && !forkColumns.has("fork_source_thread_id")) {
    yield* sql`
      ALTER TABLE projection_threads
  ...
  if (forkColumns.has("source_turn_id") && !forkColumns.has("fork_source_turn_id")) {
    yield* sql`
      ALTER TABLE projection_threads
  ...
  if (
    forkColumns.has("source_checkpoint_turn_count") &&
    !forkColumns.has("fork_source_checkpoint_turn_count")
  ) {
    yield* sql`
      ALTER TABLE projection_threads
  ...
  const repairedForkColumns = yield* getProjectionThreadColumns(sql);

  if (!repairedForkColumns.has("fork_source_thread_id")) {
    yield* sql`
      ALTER TABLE projection_threads
  ...
  if (!repairedForkColumns.has("fork_source_turn_id")) {
    yield* sql`
      ALTER TABLE projection_threads
  ...
  if (!repairedForkColumns.has("fork_source_checkpoint_turn_count")) {
    yield* sql`
      ALTER TABLE projection_threads
  ...
  if (!repairedForkColumns.has("forked_at")) {
    yield* sql`
      ALTER TABLE projection_threads
