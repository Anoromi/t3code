...
import { ensureProjectionThreadForkOriginColumns } from "./034_ProjectionThreadsForkOrigin.ts";
import { ensureProjectionProjectHyprnavColumns } from "./035_ProjectionProjectsHyprnavSettings.ts";
...
import { repairProjectionThreadLatestTurnIds } from "./039_RepairProjectionThreadLatestTurnIds.ts";
...
const previousRebuildDatabase = it.layer(SqliteClient.layerMemory());
...
freshDatabase("041_RepairForkMigrationCompatibility fresh database", (it) => {
  it.effect("applies the canonical and fork migrations on a fresh database", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(ledger, [
        ...
        { migrationId: 34, name: "ProjectionThreadsForkOrigin" },
        { migrationId: 35, name: "ProjectionProjectsHyprnavSettings" },
        { migrationId: 36, name: "NormalizeProjectHyprnavScopes" },
        { migrationId: 37, name: "RestoreInheritedProjectHyprnavNulls" },
        { migrationId: 38, name: "RepairProviderInstanceIdProjectionColumns" },
        { migrationId: 39, name: "RepairProjectionThreadLatestTurnIds" },
        { migrationId: 40, name: "ProviderSessionRuntimeIndexes" },
        { migrationId: 41, name: "RepairForkMigrationCompatibility" },
      ]);
...
previousRebuildDatabase("041_RepairForkMigrationCompatibility previous rebuild", (it) => {
  it.effect("repairs the old 33-40 ledger while preserving its historical names", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      yield* runMigrations({ toMigrationInclusive: 32 });
      ...
      assert.deepEqual(
        ...
        [[41, "RepairForkMigrationCompatibility"]],
      );

      const ledger = yield* sql<{ readonly migrationId: number; readonly name: string }>`
        SELECT migration_id AS "migrationId", name
        FROM effect_sql_migrations
        WHERE migration_id BETWEEN 33 AND 41
        ORDER BY migration_id
      `;
      assert.deepEqual(
      ...
      assert.deepEqual(ledger.at(-1), {
        migrationId: 41,
        name: "RepairForkMigrationCompatibility",
      });
...
partialForkDatabase("041_RepairForkMigrationCompatibility partial fork database", (it) => {
  it.effect("prepares canonical prerequisites before colliding fork ledgers resume", () =>
    Effect.gen(function* () {
      ...
      const ledger = yield* sql<{ readonly migrationId: number; readonly name: string }>`
        SELECT migration_id AS "migrationId", name
        FROM effect_sql_migrations
        WHERE migration_id IN (16, 17, 41)
        ORDER BY migration_id
      `;
...
authlessForkDatabase("041_RepairForkMigrationCompatibility authless fork database", (it) => {
...
cutoff23ForkDatabase("041_RepairForkMigrationCompatibility migration-23 fork database", (it) => {
...
pre29ForkDatabase("041_RepairForkMigrationCompatibility pre-29 fork database", (it) => {
...
forkDatabase("041_RepairForkMigrationCompatibility fork database", (it) => {
  it.effect("repairs a historical fork ledger without rewriting it", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(
        executed.map(([id, name]) => [id, name]),
        [
          [38, "RepairProviderInstanceIdProjectionColumns"],
          [39, "RepairProjectionThreadLatestTurnIds"],
          [40, "ProviderSessionRuntimeIndexes"],
          [41, "RepairForkMigrationCompatibility"],
        ],
      );
