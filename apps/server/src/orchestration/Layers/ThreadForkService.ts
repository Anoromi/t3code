  ...
  const createForkEvent: ThreadForkServiceShape["createForkEvent"] = ({ command, readModel }) =>
    Effect.gen(function* () {
      ...
      const messages = retainedMessages.map((message) => {
        const nextMessageId = MessageIdSchema.make(crypto.randomUUID());
      ...
      const proposedPlans = retainedProposedPlans.map((proposedPlan) => {
        const nextPlanId = OrchestrationProposedPlanIdSchema.make(crypto.randomUUID());
      ...
      const activities = retainedActivities.map((activity) => {
        const nextActivity = {
          id: EventIdSchema.make(crypto.randomUUID()),
          tone: activity.tone,
          kind: activity.kind,
